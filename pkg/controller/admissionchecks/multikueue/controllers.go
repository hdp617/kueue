...
func SetupControllers(mgr ctrl.Manager, namespace string, opts ...SetupOption) error {
	...
	if features.Enabled(features.MultiKueueClusterProfile) {
		if options.clusterProfiles != nil {
			cpRec := newCPReconciler(mgr.GetClient(), namespace)
			err = cpRec.SetupWithManager(mgr)
			if err != nil {
				return err
			}
			p := []credentials.Provider{}
			if options.clusterProfiles.CredentialsProvider != nil {
				for _, provider := range options.clusterProfiles.CredentialsProvider.Providers {
