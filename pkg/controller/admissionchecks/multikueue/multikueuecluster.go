...
func (c *clustersReconciler) Reconcile(ctx context.Context, req reconcile.Request) (reconcile.Result, error) {
	...
	if features.Enabled(features.MultiKueueClusterProfile) && cluster.Spec.ClusterProfile != nil {
		restConfig, retry, err := c.getRestConfigFromClusterProfile(ctx, cluster.Spec.ClusterProfile)
		if retry {
			return reconcile.Result{}, err
		}
		if err != nil {
			log.Error(err, "reading rest config from ClusterProfile")
			c.stopAndRemoveCluster(req.Name)
		...
		clientConfig.Kubeconfig = kubeConfig
	}
...
func (c *clustersReconciler) setupWithManager(mgr ctrl.Manager) error {
	...
	filter := predicate.Funcs{
		...
		UpdateFunc: func(ue event.UpdateEvent) bool {
			...
			if clusterNew.Spec.KubeConfig != nil && clusterNew.Spec.KubeConfig.LocationType == kueue.PathLocationType {
